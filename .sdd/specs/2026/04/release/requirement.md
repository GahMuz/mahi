...
1. QUAND les pré-conditions sont validées ALORS LA commande DOIT demander à l'utilisateur :
   "Numéro de version pour cette release (ex. 0.2.0) ?"
2. LE format de la version DOIT respecter le pattern sémantique `MAJOR.MINOR.PATCH`
   (uniquement des chiffres séparés par des points)
3. QUAND la version saisie ne respecte pas le format ALORS LA commande DOIT rejeter
   la saisie et redemander
4. LA version saisie NE DOIT PAS contenir de suffixe (`-SNAPSHOT`, `-RC`, etc.)
5. QUAND la version est validée ALORS LA commande DOIT afficher un récapitulatif :
