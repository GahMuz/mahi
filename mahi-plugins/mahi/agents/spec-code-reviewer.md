...
**Règles :**
Si des règles sont déjà présentes dans ce prompt (injectées par l'orchestrateur), les utiliser directement — ne pas recharger via sdd-rules. Sinon (invocation directe) : Glob `**/sdd-rules/SKILL.md` → lire et exécuter le protocole de chargement (plugin + projet + priorité).
