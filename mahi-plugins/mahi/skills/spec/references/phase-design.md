...
Load `.mahi/work/spec/<spec-path>/requirement.md`. Extraire :
...
Lire `.mahi/work/registry.json`. Pour chaque spec avec statut `completed` ou `retrospective` :
...
- `mcp__plugin_mahi_mahi__write_artifact`, `mcp__plugin_mahi_mahi__add_design_element` et `mcp__plugin_mahi_mahi__check_coherence` appelés après finalisation
