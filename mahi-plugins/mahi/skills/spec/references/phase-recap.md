...
> Note mahi : il n'y a pas de lecture de state.json pour obtenir la phase.
...
Exécuter `git status --short` dans le worktree (si `worktreePath` est défini dans la réponse de `mcp__plugin_mahi_mahi__get_workflow`).
