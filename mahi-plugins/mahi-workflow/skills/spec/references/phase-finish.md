...
```
> Note mahi-workflow : ne pas lire state.json pour vérifier la phase courante.
> Utiliser toujours `mahi_get_workflow` comme source de vérité.
...
> Note mahi-workflow : la mise à jour de la phase se fait via `mahi_fire_event`,
...
```
mahi_remove_worktree(workflowId: <lire depuis active.json>)
```
> Note mahi-workflow : `mahi_remove_worktree` remplace les commandes git manuelles.
