...
1. Read `.sdd/local/active.json` to get the active spec's `path` and `workflowId`. (The parent handler already failed if absent.)
...
```
mahi_fire_event(workflowId: <depuis active.json>, event="clarify", data={
