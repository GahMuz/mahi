...
Les champs suivants NE DOIVENT PAS figurer dans active.json pour mahi-workflow :
- `currentPhase` — la phase courante est lue via `mahi_get_workflow(workflowId)`, pas stockée localement
...
2. `workflowId` doit être une valeur non vide (UUID retourné par `mahi_create_workflow`)
...
2. La phase courante est lue via `mahi_get_workflow(workflowId)` — pas depuis active.json
