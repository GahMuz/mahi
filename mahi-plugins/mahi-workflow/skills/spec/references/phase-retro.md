...
> Note mahi-workflow : il n'y a pas de lecture de state.json changelog.
...
Agent({
  ...
  subagent_type: "mahi-workflow:doc-generator",
...
> Note mahi-workflow : il n'y a pas de mise à jour de state.json.
> La transition vers `completed` est déclenchée via `mahi_fire_event`.
