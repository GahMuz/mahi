...
> Note mahi-workflow : il n'y a pas de lecture de state.json pour obtenir la phase ou la progression.
...
Agent({
  ...
  subagent_type: "mahi-workflow:spec-orchestrator",
  ...
  prompt: "Spec: <spec-id>
    ...
    Ne pas lire state.json — utiliser mahi_get_workflow pour l'état courant."
})
```
...
- Vérifier la progression via `mahi_get_workflow(workflowId)` — ne pas lire state.json
...
Agent({
  ...
  subagent_type: "mahi-workflow:spec-reviewer",
